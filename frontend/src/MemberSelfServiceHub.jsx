      ...
      {activeTab === 'overview' && (
        <div className="space-y-3">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            <SectionShell
              title="Pending dues"
              subtitle={(dashboard?.payments_summary?.pending_total || 0) > 0
                ? 'You can clear outstanding balances here before your next visit.'
                : 'No outstanding balances were found on your account.'}
            >
              {(dashboard?.payments_summary?.pending_total || 0) > 0 ? (
                <div className="space-y-3">
                  <div className="rounded-2xl border border-amber-300/20 bg-amber-500/10 px-4 py-4">
                    <p className="text-[10px] font-black uppercase tracking-widest text-amber-200">Outstanding</p>
                    <p className="text-white font-black text-3xl mt-2">₹{formatCollectionAmount(dashboard?.payments_summary?.pending_total || 0)}</p>
                    <p className="text-amber-100/80 text-sm font-semibold mt-2">
                      {dashboard?.payments_summary?.pending_count || 0} pending record{(dashboard?.payments_summary?.pending_count || 0) === 1 ? '' : 's'} need attention.
                    </p>
                  </div>
                  <button
                    type="button"
                ...
                <div className="rounded-2xl border border-emerald-300/20 bg-emerald-500/10 px-4 py-4 text-emerald-100 text-sm font-semibold">
                  Your payment ledger is clear right now.
                </div>
              )}
            </SectionShell>
            ...
            <SectionShell
              ...
              subtitle={canBookClasses ? 'Your next bookings appear here. Jump to Classes to book more sessions.' : 'Class booking unlocks when your membership is active.'}
            >
              {upcomingSummaryBookings.length > 0 ? (
                <div className="space-y-2">
                  {upcomingSummaryBookings.map((booking) => (
                    <div key={booking.id} className="rounded-2xl border border-white/10 bg-white/5 px-4 py-3">
                      <div className="flex items-start justify-between gap-3">
                        <div>
                          <p className="text-white font-black text-sm">{booking.class_title}</p>
                          <p className="text-slate-400 text-xs font-semibold mt-1">{formatDateTime(booking.starts_at)}</p>
                        </div>
                        <span className="px-2.5 py-1 rounded-full text-[10px] font-black uppercase tracking-wider bg-indigo-500/20 text-indigo-200">
                          {booking.status}
                        </span>
                      </div>
                    </div>
          ...
          <SectionShell
            ...
            subtitle="This is your latest membership and due-collection ledger activity."
          >
